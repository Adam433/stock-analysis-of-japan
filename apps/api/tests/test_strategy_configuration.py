...
class StrategyConfigurationTests(unittest.TestCase):
    ...
    def test_get_active_strategy_configuration_creates_default_configuration(self) -> None:
        ...
        self.assertEqual(configuration.selected_rps_windows, [50, 120, 250])
        self.assertEqual(configuration.min_rps_lines_required, 1)
    ...
    def test_save_strategy_configuration_creates_new_active_version(self) -> None:
        ...
        self.assertEqual(updated_configuration.selected_rps_windows, [50, 250])
        self.assertEqual(updated_configuration.min_rps_lines_required, 2)
        ...
        with self.session_factory() as session:
            ...
            with self.assertRaises(ValueError):
                save_strategy_configuration(
                    session,
                    rps_threshold=90,
                    selected_rps_windows=[APPROVED_RPS_WINDOWS[0]],
                    min_rps_lines_required=2,
                    high_proximity_threshold_pct=Decimal("5.00"),
                )
