    ...
    for provider_row in provider_rows[-5:]:
        ...
        if row is None:
            row = FundamentalsAnnual(
                ...
                operating_cash_flow=provider_row.operating_cash_flow,
                free_cash_flow=provider_row.free_cash_flow,
                diluted_eps=provider_row.diluted_eps,
                stockholders_equity=provider_row.stockholders_equity,
                weighted_average_diluted_shares=provider_row.weighted_average_diluted_shares,
                pe=provider_row.pe,
                pb=provider_row.pb,
...
def _apply_provider_row(row: FundamentalsAnnual, provider_row: ProviderFundamentalsAnnual) -> None:
    ...
    row.operating_cash_flow = provider_row.operating_cash_flow
    row.free_cash_flow = provider_row.free_cash_flow
    row.diluted_eps = provider_row.diluted_eps
    row.stockholders_equity = provider_row.stockholders_equity
    row.weighted_average_diluted_shares = provider_row.weighted_average_diluted_shares
