...
MAX_WORKERS = int(os.environ.get("STOCKANALYSE_MARKET_FILTER_D2_WORKERS", "2"))
MAX_TASKS_PER_CHILD = int(
    os.environ.get("STOCKANALYSE_MARKET_FILTER_D2_MAX_TASKS_PER_CHILD", "24")
)
MARKET_FILTER_SYMBOL = os.environ.get("STOCKANALYSE_MARKET_FILTER_D2_SYMBOL", "QQQ").upper()
...
def _price_sma_filter(days: int) -> dict[str, object]:
    return {
        "enabled": True,
...
def _market_filters() -> list[tuple[str, dict[str, object]]]:
    return [
        ...
        ("qqq_100ma", _price_sma_filter(100)),
        ("qqq_150ma", _price_sma_filter(150)),
        ("qqq_200ma", _price_sma_filter(200)),
    ]
...
def main() -> None:
    ...
    design = {
        "name": "market_filter_experiment_d2",
        "description": (
            "Experiment D2: QQQ-based loose market-regime filter test across the same "
            "24 core strategy cells used by Experiment D."
        ),
        "market_filters": ["none", "qqq_100ma", "qqq_150ma", "qqq_200ma"],
