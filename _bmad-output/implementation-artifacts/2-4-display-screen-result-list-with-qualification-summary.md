# 故事 2.4: Display Screen Result List with Qualification Summary
...
以便I can choose which candidates to inspect further。

## 验收标准

1. 假设a completed screen run with qualifying securities，当the user views the result list，那么the system displays the qualified stocks from that run。
2. 假设a completed screen run with qualifying securities，当the user views the result list，那么the result list is associated with the run date and parameter set that produced it。
3. 假设a stock appears in the result list，当the user views its summary，那么the summary indicates that the stock passed and provides enough context to open the detailed explanation flow。
