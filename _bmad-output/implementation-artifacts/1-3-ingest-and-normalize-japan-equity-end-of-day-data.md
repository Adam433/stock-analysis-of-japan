# 故事 1.3: Ingest and Normalize Japan Equity End-of-Day Data
...
以便screening and backtesting run on a trustworthy dataset。

## 验收标准

1. 假设a configured market-data provider，当a market-data refresh is executed，那么the system stores normalized Japan equity instrument records and daily market data records。
2. 假设a configured market-data provider，当a market-data refresh is executed，那么raw-price and adjusted-price concepts remain distinct in storage。
3. 假设provider data contains incomplete or unavailable values，当normalization runs，那么the system marks incomplete or unavailable states explicitly instead of silently treating them as complete data。
