# 故事 2.2: Materialize RPS and 52-Week-High Derived Facts
...
作为系统，  
我希望compute and persist derived screening facts from normalized market data，  
以便screening, charting, and backtesting all use the same authoritative values。

## 验收标准

1. 假设normalized daily market data exists，当derived-facts materialization runs，那么the system computes and stores 50-day, 120-day, and 250-day RPS-related values for supported securities。
2. 假设normalized daily market data exists，当derived-facts materialization runs，那么the system computes and stores 52-week-high proximity values for supported securities。
3. 假设derived facts have been computed，当later services query them，那么the same stored values are available for screening, chart detail, and backtesting。
